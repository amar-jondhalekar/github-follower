...
const App = () => {
  ...
  // New function to fetch users based on keyword and display them
  const fetchUsersByKeyword = async () => {
    setMessage("");
    setUsers([]); // Clear previous users data

    try {
      const searchUrl = `https://api.github.com/search/users?q=${input}&per_page=100`;
      const searchResponse = await axios.get(searchUrl);
      const usersArray = searchResponse.data.items;
      setUsers(usersArray); // Set users data to display
      setMessage(`${usersArray.length} users found matching your keyword.`);
    } catch (error) {
      setMessage(`Error fetching users: ${error.message}`);
    }
  };
    ...
    <div className="container">
      ...
      <div className="input-group">
        <input
          ...
          placeholder={
            followingMode === "usernames"
              ? "Enter GitHub usernames (comma-separated)"
              : "Enter keyword (e.g., Web developer)"
          }
        />
        <button onClick={handleFollowMultiple}>Follow Users</button>
        <button className="stop-button" onClick={stopFollowing}>
          Stop
        </button>
        <button className="search-button" onClick={fetchUsersByKeyword}>
        ...
        <div className="user-cards">
          {users.map((user) => (
            <div key={user.id} className="user-card">
              ...
              <div>
                <h3>{user.login}</h3>
                <button onClick={() => handleViewProfile(user.login)}>
                  View Profile
                </button>
              </div>
            </div>
            ...
            <div className="modal-body">
              ...
              <div className="modal-info">
                ...
                <ul>
                  ...
                  <li>
                    <strong>Public Repos:</strong> {selectedUser.public_repos}
                  </li>
                  <li>
